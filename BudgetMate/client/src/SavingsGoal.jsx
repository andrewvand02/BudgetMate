...
const SavingsGoal = () => {
    ...
    return (
        <div className="savings-goal-container"> {/* Container for the savings goal page */}
            <h1>Your Savings Goal</h1> {/* Page header */}
            <form onSubmit={handleSubmit}> {/* Form to submit savings goal */}
                <div className="goal-input">
                    <label>
                        Set Goal Amount ($):
                        <input 
                            type="number"
                            value={savingsGoal} // Bind the input value to the savingsGoal state
                            onChange={handleGoalChange} // Handle changes to the savings goal
                            placeholder="Enter your savings goal" // Placeholder text for the input field
                            min="0" // Ensure the goal is non-negative
                        />
                    </label>
                    <label>
                        Start Week:
                        <input 
                            ...
                            onChange={handleStartDateChange} // Handle changes to the start date
                            min={new Date().toISOString().split('T')[0]} // Ensure the start date is not in the past
                        />
                    </label>
                    <label>
                        End Week:
                        <input 
                            ...
                            onChange={handleEndDateChange} // Handle changes to the end date
                            min={startDate} // Ensure the end date is after the start date
                        />
                    </label>
                </div>
                ...
                <div className="avg-weekly-savings">
                    {avgWeeklySavings > 0 ? (
                        <p>Average Weekly Savings Needed: ${avgWeeklySavings.toFixed(2)}</p> // Display average weekly savings if available
                    ) : (
                        <p>Set a goal and target date to calculate the required weekly savings.</p> // Prompt user to set a goal and dates
                    )}
                </div>
                <button type="submit" className="button type1">
                    <span className="btn-txt">Save Goal</span> {/* Button to submit the form */}
                </button>
            </form>
...
export default SavingsGoal; // Export the SavingsGoal component for use in other parts of the app
